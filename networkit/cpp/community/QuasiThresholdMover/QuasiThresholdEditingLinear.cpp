...
namespace NetworKit {
	namespace QuasiThresholdMoving {
		QuasiThresholdEditingLinear::QuasiThresholdEditingLinear(const Graph &G) : G(G), hasRun(false) {
		}
		
		void QuasiThresholdEditingLinear::run() {
		...
		std::vector< node > QuasiThresholdEditingLinear::getParents() const {
			return parent;
		}
		
		
		Graph QuasiThresholdEditingLinear::getDefiningForest() const {
		...
		Graph QuasiThresholdEditingLinear::getQuasiThresholdGraph() const {
