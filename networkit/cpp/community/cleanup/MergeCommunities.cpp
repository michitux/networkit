...
#pragma omp for schedule(dynamic, 10) nowait
        for (omp_index commId = 0; commId < numComms; ++commId) {
            ...
            neighborCommunities.forElements([&](index neighborCommunity, count weight) {
...
bool MergeCommunities::tryLocalMove(node u, SparseVector<edgeweight> &neighborWeights) {
    ...
    neighborWeights.forElements([&](index neighborCommunity, double numEdgesIntoCommunity) {
        double score = 1.;
...
#pragma omp atomic
                ...
                std::vector<index> nodesOfMergedCommunity;
                nodesOfMergedCommunity.reserve(mergedCommunity.size());
                mergedCommunity.forElements([&nodesOfMergedCommunity](node u, bool) {
                    nodesOfMergedCommunity.push_back(u);
                });
