...
namespace NetworKit {
	...
	void CKBDynamic::BucketSampling::verifyInvariants(bool verify_oversampling) const {
	...
	CKBDynamic::BucketSampling::BucketSampling(count n, count minSlots, count maxSlots, double exponent) : pl_generator(minSlots, maxSlots, exponent), nodes_by_memberships(maxSlots + 1), nodes_with_fractional_slots_for_degree(maxSlots + 1), sumOfDesiredMemberships(0) {
		pl_generator.run();
	...
	void CKBDynamic::BucketSampling::assignCommunity(node nodeId) {
	...
	void CKBDynamic::BucketSampling::leaveCommunity(node nodeId) {
	...
	void CKBDynamic::BucketSampling::removeFraction(node nodeId, count fraction) {
	...
	void CKBDynamic::BucketSampling::addFraction(node nodeId, count fraction) {
		if (fraction == 0) return;
	...
	node CKBDynamic::BucketSampling::addNode(count degree, bool oversample) {
	...
	node CKBDynamic::BucketSampling::addNode() {
		return addNode(pl_generator.getDegree());
	}

	node CKBDynamic::BucketSampling::addNode(node degree) {
	...
	node CKBDynamic::BucketSampling::removeNode(node nodeId, bool withFraction) {
	...
	node CKBDynamic::BucketSampling::removeNode(node nodeId) {
